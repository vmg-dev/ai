...
3. **Core Type System**  
   The core `AI` class uses generic type helpers to extract model-specific options:
...
4. **Method Signatures**  
   The `chat` and `chatCompletion` methods are generic in `TModel` and constrain `providerOptions` accordingly:

   ```typescript
   chat<TModel extends ExtractModels<TAdapter>>(
     params: ChatCompletionOptions<
       TModel,
       ExtractChatProviderOptions<TAdapter>,
       undefined,
       ExtractModelSpecificProviderOptions<TAdapter, TModel>  // <-- Model-specific narrowing
     > & { model: TModel }
   )
   ```
...
const validCall = aiInstance.chat({
  model: "gpt-5",
  messages: [],
  providerOptions: {
    text: {  // OK - OpenAIStructuredOutputOptions is included for gpt-5
      type: "json_schema",
      json_schema: { /* ... */ }
    }
  }
...
const invalidCall = aiInstance.chat({
  model: "gpt-4-turbo",
  messages: [],
  providerOptions: {
...
1. **Add model metadata** with capabilities in `model-meta.ts`:
   ...
   export const NEW_MODEL_META = {
     // ... metadata
   } as const satisfies ModelMeta<OpenAIBaseOptions & OpenAIStructuredOutputOptions>;
   ```
...
2. **Update the type map** in `OpenAIChatModelProviderOptionsByName`:
   ...
   export type OpenAIChatModelProviderOptionsByName = {
     // ... existing models
     "new-model": OpenAIBaseOptions & OpenAIStructuredOutputOptions & OpenAIToolsOptions & OpenAIStreamingOptions & OpenAIMetadataOptions;
   };
   ```
...
The `_modelProviderOptionsByName` property on adapters is a **type-only** property (definite assignment `!`). It exists purely for TypeScript's type system and is never accessed at runtime. This allows the core `AI` class to extract model-specific types via indexed access without runtime overhead.
